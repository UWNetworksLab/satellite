// node ./country_by-domain-ips.js country-domain-ip.json scores.json output.json [threshold]
//
...
function doCountry(scores, line) {
  var result,
    table;
...
function doAll(scores) {
  ...
  return Q.Promise(function (resolve, reject) {
    fs.createReadStream(inFile)
      .pipe(es.split())
      .pipe(es.mapSync(doCountry.bind({}, scores)))
      .on('end', resolve)
      .on('error', reject);
  }).then(function () {
